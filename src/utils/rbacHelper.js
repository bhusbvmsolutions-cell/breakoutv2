...
const userHasPermission = (userWithRoles, module, action) => {
  ...
  for (const role of roles) {
    ...
    for (const permission of permissions) {
      ...
      const permissionModule = (permission.module || '').toString();
      const permissionAction = normalizeAction(permission.action);

      if (permissionModule === 'system' && permissionAction === 'manage') {
        return true;
      }

      if (
        permissionModule === module &&
        (permissionAction === normalizedAction || permissionAction === 'manage')
      ) {
        return true;
      }
...
const filterAssignableRoles = (roles = [], maxLevel, superAdmin) => {
  ...
  return roles.filter(role =>
    Number(role.level || 0) < Number(maxLevel || 0)
  );
};
