...
const VenueController = {
  ...
  create: async (req, res) => {
    try {
      const locations = await db.Location.findAll({ where: { isActive: true }, order: [['title', 'ASC']] });
      const categories = await db.VenueCategory.findAll({ where: { isActive: true }, order: [['name', 'ASC']] });
      const experienceTypes = await db.VenueExperienceType.findAll({ where: { isActive: true }, order: [['name', 'ASC']] });
      const lookingFor = await db.VenueExperienceLookingFor.findAll({ where: { isActive: true }, order: [['name', 'ASC']] });
      const partyTypes = await db.VenuePartType.findAll({ where: { isActive: true }, order: [['name', 'ASC']] });
      const suitableTimes = await db.VenueSuitableTime.findAll({ where: { isActive: true }, order: [['name', 'ASC']] });
      const budgetRanges = await db.VenueBudgetRange.findAll({ where: { isActive: true }, order: [['name', 'ASC']] });
  ...
  store: async (req, res) => {
    ...
    try {
      ...
      const existingSlug = await db.Venue.findOne({ where: { slug }, transaction });
      ...
      const assocMappings = [
        { input: body.categories, model: db.VenueCategoryMapping, field: 'category_id' },
        { input: body.experienceTypes, model: db.VenueExperienceTypeMapping, field: 'experience_type_id' },
        { input: body.lookingFor, model: db.VenueExperienceLookingForMapping, field: 'looking_for_id' },
        { input: body.partyTypes, model: db.VenuePartyTypeMapping, field: 'party_type_id' },
        { input: body.suitableTimes, model: db.VenueSuitableTimeMapping, field: 'suitable_time_id' },
        { input: body.budgetRanges, model: db.VenueBudgetRangeMapping, field: 'budget_range_id' },
      ];
  ...
  update: async (req, res) => {
    ...
    try {
      ...
      if (updateData.slug !== venue.slug) {
        const existing = await db.Venue.findOne({ where: { slug: updateData.slug, id: { [Op.ne]: venue.id } }, transaction });
      ...
      const assocModels = [
        { model: db.VenueCategoryMapping, idField: 'category_id', inputName: 'categories' },
        { model: db.VenueExperienceTypeMapping, idField: 'experience_type_id', inputName: 'experienceTypes' },
        { model: db.VenueExperienceLookingForMapping, idField: 'looking_for_id', inputName: 'lookingFor' },
        { model: db.VenuePartyTypeMapping, idField: 'party_type_id', inputName: 'partyTypes' },
        { model: db.VenueSuitableTimeMapping, idField: 'suitable_time_id', inputName: 'suitableTimes' },
        { model: db.VenueBudgetRangeMapping, idField: 'budget_range_id', inputName: 'budgetRanges' },
      ];
      for (const assoc of assocModels) {
        await assoc.model.destroy({ where: { venue_id: venue.id }, transaction });
      ...
      // Gallery images
      await db.VenueImage.destroy({ where: { venue_id: venue.id }, transaction });
      if (body.gallery_images && Array.isArray(body.gallery_images)) {
        for (let i = 0; i < body.gallery_images.length; i++) {
          const img = body.gallery_images[i];
          const imageFile = files[`gallery_images[${i}][image_file]`]?.[0];
  ...
  delete: async (req, res) => {
    ...
    try {
      ...
      const galleryImages = await db.VenueImage.findAll({ where: { venue_id: venue.id }, transaction });
      ...
      await transaction.rollback();
      console.error(error);
      res.status(500).json({ success: false, message: error.message || 'Failed to delete venue' });
    }
  },
