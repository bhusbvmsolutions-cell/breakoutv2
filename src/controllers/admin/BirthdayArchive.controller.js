  ...
  index: async (req, res) => {
    try {
      ...
      const fullArchive = await db.BirthdayArchive.findByPk(archive.id, {
        include: [
          { model: db.BirthdayArchiveCounterCard, as: 'counterCards', order: [['sort_order', 'ASC']] },
          { model: db.BirthdayArchiveInclusionItem, as: 'inclusionItems', order: [['sort_order', 'ASC']] },
          { model: db.BirthdayArchiveSliderItem, as: 'sliderItems', order: [['sort_order', 'ASC']] },
    ...
    try {
      ...
      for (const field of imageFields) {
        if (files[field] && files[field][0]) {
          ...
          updateData[field] = `/uploads/party/birthday/${files[field][0].filename}`;
        } else if (body[field]) {
      ...
      // Video testimonials
      await db.BirthdayArchiveVideo.destroy({ where: { archive_id: archive.id }, transaction });
