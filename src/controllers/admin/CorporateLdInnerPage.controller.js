...
const CorporateLdInnerPageController = {
  ...
  index: async (req, res) => {
    try {
      const pages = await db.CorporateLdInnerPage.findAll({
        ...
        include: [
          { model: db.Video, as: "videos", through: { attributes: [] } },
        ],
      });
  ...
  create: async (req, res) => {
    try {
      ...
      res.render("admin/corporate/ld/inner/create", {
        title: "Add Corporate L&D Inner Page",
        videos,
        page: null,
        errors: {},
      });
  ...
  store: async (req, res) => {
    ...
    try {
      ...
      const existingSlug = await db.CorporateLdInnerPage.findOne({
        where: { slug },
        transaction,
      });
      ...
      const page = await db.CorporateLdInnerPage.create(pageData, {
        transaction,
      });
        ...
        if (items && Array.isArray(items)) {
          for (let i = 0; i < items.length; i++) {
            ...
            if (fileMap && fileMap.field) {
              ...
              if (file) {
                data[fileMap.dest] =
                  `/uploads/corporate-ld-inner/${file.filename}`;
      ...
      // Video testimonials
      await db.CorporateLdInnerVideo.destroy({
  ...
  edit: async (req, res) => {
    try {
      const page = await db.CorporateLdInnerPage.findByPk(req.params.id, {
        include: [
          {
            model: db.CorporateLdInnerImageCard,
            as: "imageCards",
            order: [["sort_order", "ASC"]],
          },
          {
            model: db.CorporateLdInnerPoint,
            as: "points",
            order: [["sort_order", "ASC"]],
          },
          {
            model: db.CorporateLdInnerKeyResource,
            as: "keyResources",
            order: [["sort_order", "ASC"]],
          },
          {
      ...
      res.render("admin/corporate/ld/inner/edit", {
        title: "Edit Corporate L&D Inner Page",
        page,
        videos,
        errors: {},
      });
  ...
  update: async (req, res) => {
    ...
    try {
      ...
      if (updateData.slug !== page.slug) {
        const existing = await db.CorporateLdInnerPage.findOne({
          where: {
            slug: updateData.slug,
            id: { [db.Sequelize.Op.ne]: page.id },
          },
          transaction,
        });
