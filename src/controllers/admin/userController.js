...
const getPrimaryRoleName = (roles = [], roleIds = []) => {
  ...
  const roleIdSet = new Set(
    roleIds.map((id) => Number.parseInt(id, 10)).filter(Number.isInteger),
  );
...
const userController = {
  ...
  listUsers: async (req, res) => {
    try {
      ...
      const canCreateUser =
        isSuper || userHasPermission(currentUser, "users", "create");
      const canUpdateUser =
        isSuper || userHasPermission(currentUser, "users", "update");
      const canDeleteUser =
        isSuper || userHasPermission(currentUser, "users", "delete");

      const assignableRoles = filterAssignableRoles(
        allRoles,
        currentUserLevel,
        isSuper,
      );
      ...
      const usersWithManage = users.map((user) => {
        ...
        userJson.canManage =
          canUpdateUser && (isSuper || currentUserLevel > userMaxLevel);
        userJson.canDelete =
          canDeleteUser &&
          userJson.canManage &&
          user.id !== req.session.user.id; // Can't delete self
        return userJson;
      });
  ...
  createUser: async (req, res) => {
    ...
    try {
      const { firstName, lastName, username, email, password, phone, roleIds } =
        req.body;
      ...
      if (
        !isSuper &&
        requestedRoleIds.some((roleId) => !assignableRoleIds.has(roleId))
      ) {
      ...
      if (isSuper && requestedRoleIds.length > 0) {
        ...
        if (roleCount !== new Set(requestedRoleIds).size) {
          await transaction.rollback();
          req.flash("error", "One or more selected roles do not exist");
          return res.render("admin/users/create", {
            title: "Create User",
            roles: assignableRoles,
            formData: { ...req.body, username: (username || "").trim() },
      ...
      const selectedRoleIds = normalizeIdArray(req.body.roleIds).map((id) =>
        id.toString(),
      );
  ...
  editUserForm: async (req, res) => {
    try {
      ...
      if (!isSuper && currentUserLevel <= targetUserLevel) {
        req.flash("error", "You cannot edit this user");
  ...
  updateUser: async (req, res) => {
    ...
    try {
      const { id } = req.params;
      const { firstName, lastName, username, phone, isActive, roleIds } =
        req.body;
      ...
      if (normalizedUsername) {
        ...
        if (existingUsername) {
          await transaction.rollback();
          return res.redirect(
            `/admin/users/${id}/edit?error=Username already in use`,
          );
        }
      }
      ...
      if (requestedRoleIds.length > 0) {
        ...
        if (roles.length !== new Set(requestedRoleIds).size) {
          await transaction.rollback();
          return res.redirect(
            `/admin/users/${id}/edit?error=One or more selected roles do not exist`,
          );
        }

        if (
          !isSuper &&
          roles.some((role) => (role.level || 0) >= currentUserLevel)
        ) {
          await transaction.rollback();
          return res.redirect(
            `/admin/users/${id}/edit?error=You cannot assign roles higher than your own level`,
          );
        }
